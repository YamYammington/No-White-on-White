...
// key/value pairs of color names and hex values
function colorNameToRGB(color) {
    ...
    if (typeof colors[color.toLowerCase()] != 'undefined') {
        return hexToRgbArray(colors[color.toLowerCase()]);
    }
...
function invert(color) {
    ...
    if (color.includes("#")) {  // check if the color is a hex value
        var rgb = hexToRgbArray(color);

    } else if (!/\d/.test(color)) {  // check if the color contains a number
        var rgb = colorNameToRGB(color);

    } else {  // get the hsl/rgb values and convert to hex
        ...
        if (match) {
            if (match[1].includes("hsl")) { // convert HSL values to RGB
                var rgb = HSLToRGB(match[2], match[3].slice(0, -1), match[4].slice(0, -1));  // slice for removing percentage signs
            } else {
...
window.onload = function () {
    ...
    invertButton.onclick = function () {
        ...
        if (invertedColor) {
            ...
            console.log(`%c${invertedColor}`, `color: ${invertedColor}; background-color: ${color}`);
...
// get the background color of all elements that contain text, and the text color of those text elements. if the colors match, invert the text color.
function main() {
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        if (bgColor === textColor) {
            element.style.color = invert(textColor);
        }
    }
}
*/